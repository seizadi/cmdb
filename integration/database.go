...
//GetAsArgs returns the slice with set of arguments for the test Postgres database
func (cfg PostgresDBConfig) GetAsArgs() []string {
	return []string{"--database.address", cfg.Host, "--database.port", strconv.Itoa(cfg.Port), "--database.name", cfg.DBName, "--database.user", cfg.DBUser, "--database.password", cfg.DBPassword}
}
