// Code generated by protoc-gen-gogo. DO NOT EDIT.
// source: github.com/seizadi/cmdb/pkg/pb/cmdb.proto

package pb // import "github.com/seizadi/cmdb/pkg/pb"

import options "github.com/infobloxopen/protoc-gen-atlas-query-validate/options"
import query "github.com/infobloxopen/atlas-app-toolkit/query"
import _ "github.com/golang/protobuf/ptypes/empty"
import _ "github.com/infobloxopen/protoc-gen-atlas-validate/options"
import _ "google.golang.org/genproto/protobuf/field_mask"

// Reference imports to suppress errors if they are not otherwise used.

var CmdbMethodsRequireFilteringValidation = map[string]map[string]options.FilteringOption{
	"/api.cmdb.CloudProviders/List": map[string]options.FilteringOption{
		"id":          options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":        options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description": options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"provider":    options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"account":     options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"regions":     options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
	},
	"/api.cmdb.Regions/List": map[string]options.FilteringOption{
		"id":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":              options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":       options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"stages":            options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"aws_services":      options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"value_id":          options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"cloud_provider_id": options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.Stages/List": map[string]options.FilteringOption{
		"id":           options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":         options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":  options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"type":         options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"environments": options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"value_id":     options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"region_id":    options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.Environments/List": map[string]options.FilteringOption{
		"id":                    options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":                  options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":           options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"application_instances": options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"value_id":              options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"stage_id":              options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.AppRegionConfigs/List": map[string]options.FilteringOption{
		"id":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":              options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":       options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"app_stage_configs": options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"value_id":          options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"region_id":         options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.AppStageConfigs/List": map[string]options.FilteringOption{
		"id":                      options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":                    options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":             options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"app_environment_configs": options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"value_id":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"stage_id":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.AppEnvironmentConfigs/List": map[string]options.FilteringOption{
		"id":             options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":           options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":    options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"value_id":       options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"environment_id": options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.ChartVersions/List": map[string]options.FilteringOption{
		"id":          options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":        options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description": options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"repo":        options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"version":     options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.ApplicationInstances/List": map[string]options.FilteringOption{
		"id":               options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":             options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":      options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"deployment":       options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"value_id":         options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"chart_version_id": options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"environment_id":   options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.Vaults/List": map[string]options.FilteringOption{
		"id":          options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":        options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description": options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"secrets":     options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
	},
	"/api.cmdb.Secrets/List": map[string]options.FilteringOption{
		"id":                  options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":         options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"path":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"type":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"key":                 options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"vault_id":            options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"value_id":            options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"aws_rds_instance_id": options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.AwsServices/List": map[string]options.FilteringOption{
		"id":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":              options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":       options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"aws_rds_instances": options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"region_id":         options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.AwsRdsInstances/List": map[string]options.FilteringOption{
		"id":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":              options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":       options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"database_host":     options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"database_name":     options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"database_user":     options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"database_password": options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"aws_service_id":    options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.Values/List": map[string]options.FilteringOption{
		"id":                  options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":                options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":         options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"keys.*":              options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"aws_rds_instance_id": options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.Artifacts/List": map[string]options.FilteringOption{
		"id":               options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":             options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":      options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"repo":             options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"commit":           options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"chart_version_id": options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.KubeClusters/List": map[string]options.FilteringOption{
		"id":          options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":        options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description": options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
	"/api.cmdb.Deployments/List": map[string]options.FilteringOption{
		"id":                      options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"name":                    options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"description":             options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"artifact":                options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"artifact_id":             options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"kube_cluster":            options.FilteringOption{Deny: []options.QueryValidate_FilterOperator{options.QueryValidate_ALL}, ValueType: options.QueryValidate_DEFAULT},
		"kube_cluster_id":         options.FilteringOption{ValueType: options.QueryValidate_STRING},
		"application_instance_id": options.FilteringOption{ValueType: options.QueryValidate_STRING},
	},
}
var CmdbMethodsRequireSortingValidation = map[string][]string{
	"/api.cmdb.CloudProviders/List": []string{
		"id",
		"name",
		"description",
		"provider",
		"account",
	},
	"/api.cmdb.Regions/List": []string{
		"id",
		"name",
		"description",
		"value_id",
		"cloud_provider_id",
	},
	"/api.cmdb.Stages/List": []string{
		"id",
		"name",
		"description",
		"type",
		"value_id",
		"region_id",
	},
	"/api.cmdb.Environments/List": []string{
		"id",
		"name",
		"description",
		"value_id",
		"stage_id",
	},
	"/api.cmdb.AppRegionConfigs/List": []string{
		"id",
		"name",
		"description",
		"value_id",
		"region_id",
	},
	"/api.cmdb.AppStageConfigs/List": []string{
		"id",
		"name",
		"description",
		"value_id",
		"stage_id",
	},
	"/api.cmdb.AppEnvironmentConfigs/List": []string{
		"id",
		"name",
		"description",
		"value_id",
		"environment_id",
	},
	"/api.cmdb.ChartVersions/List": []string{
		"id",
		"name",
		"description",
		"repo",
		"version",
	},
	"/api.cmdb.ApplicationInstances/List": []string{
		"id",
		"name",
		"description",
		"value_id",
		"chart_version_id",
		"environment_id",
	},
	"/api.cmdb.Vaults/List": []string{
		"id",
		"name",
		"description",
	},
	"/api.cmdb.Secrets/List": []string{
		"id",
		"name",
		"description",
		"path",
		"type",
		"key",
		"vault_id",
		"value_id",
		"aws_rds_instance_id",
	},
	"/api.cmdb.AwsServices/List": []string{
		"id",
		"name",
		"description",
		"region_id",
	},
	"/api.cmdb.AwsRdsInstances/List": []string{
		"id",
		"name",
		"description",
		"database_host",
		"database_name",
		"database_user",
		"aws_service_id",
	},
	"/api.cmdb.Values/List": []string{
		"id",
		"name",
		"description",
		"keys",
		"aws_rds_instance_id",
	},
	"/api.cmdb.Artifacts/List": []string{
		"id",
		"name",
		"description",
		"repo",
		"commit",
		"chart_version_id",
	},
	"/api.cmdb.KubeClusters/List": []string{
		"id",
		"name",
		"description",
	},
	"/api.cmdb.Deployments/List": []string{
		"id",
		"name",
		"description",
		"artifact_id",
		"kube_cluster_id",
		"application_instance_id",
	},
}
var CmdbMethodsRequireFieldSelectionValidation = map[string][]string{
	"/api.cmdb.CloudProviders/Read": {
		"id",
		"name",
		"description",
		"provider",
		"account",
		"regions.id",
		"regions.name",
		"regions.description",
		"regions.value_id",
		"regions.cloud_provider_id",
		"regions",
	},
	"/api.cmdb.CloudProviders/List": {
		"id",
		"name",
		"description",
		"provider",
		"account",
		"regions.id",
		"regions.name",
		"regions.description",
		"regions.value_id",
		"regions.cloud_provider_id",
		"regions",
	},
	"/api.cmdb.Regions/Read": {
		"id",
		"name",
		"description",
		"stages.id",
		"stages.name",
		"stages.description",
		"stages.type",
		"stages.value_id",
		"stages.region_id",
		"stages",
		"aws_services.id",
		"aws_services.name",
		"aws_services.description",
		"aws_services.region_id",
		"aws_services",
		"value_id",
		"cloud_provider_id",
	},
	"/api.cmdb.Regions/List": {
		"id",
		"name",
		"description",
		"stages.id",
		"stages.name",
		"stages.description",
		"stages.type",
		"stages.value_id",
		"stages.region_id",
		"stages",
		"aws_services.id",
		"aws_services.name",
		"aws_services.description",
		"aws_services.region_id",
		"aws_services",
		"value_id",
		"cloud_provider_id",
	},
	"/api.cmdb.Stages/Read": {
		"id",
		"name",
		"description",
		"type",
		"environments.id",
		"environments.name",
		"environments.description",
		"environments.value_id",
		"environments.stage_id",
		"environments",
		"value_id",
		"region_id",
	},
	"/api.cmdb.Stages/List": {
		"id",
		"name",
		"description",
		"type",
		"environments.id",
		"environments.name",
		"environments.description",
		"environments.value_id",
		"environments.stage_id",
		"environments",
		"value_id",
		"region_id",
	},
	"/api.cmdb.Environments/Read": {
		"id",
		"name",
		"description",
		"application_instances.id",
		"application_instances.name",
		"application_instances.description",
		"application_instances.value_id",
		"application_instances.chart_version_id",
		"application_instances.environment_id",
		"application_instances",
		"value_id",
		"stage_id",
	},
	"/api.cmdb.Environments/List": {
		"id",
		"name",
		"description",
		"application_instances.id",
		"application_instances.name",
		"application_instances.description",
		"application_instances.value_id",
		"application_instances.chart_version_id",
		"application_instances.environment_id",
		"application_instances",
		"value_id",
		"stage_id",
	},
	"/api.cmdb.AppRegionConfigs/Read": {
		"id",
		"name",
		"description",
		"app_stage_configs.id",
		"app_stage_configs.name",
		"app_stage_configs.description",
		"app_stage_configs.value_id",
		"app_stage_configs.stage_id",
		"app_stage_configs",
		"value_id",
		"region_id",
	},
	"/api.cmdb.AppRegionConfigs/List": {
		"id",
		"name",
		"description",
		"app_stage_configs.id",
		"app_stage_configs.name",
		"app_stage_configs.description",
		"app_stage_configs.value_id",
		"app_stage_configs.stage_id",
		"app_stage_configs",
		"value_id",
		"region_id",
	},
	"/api.cmdb.AppStageConfigs/Read": {
		"id",
		"name",
		"description",
		"app_environment_configs.id",
		"app_environment_configs.name",
		"app_environment_configs.description",
		"app_environment_configs.value_id",
		"app_environment_configs.environment_id",
		"app_environment_configs",
		"value_id",
		"stage_id",
	},
	"/api.cmdb.AppStageConfigs/List": {
		"id",
		"name",
		"description",
		"app_environment_configs.id",
		"app_environment_configs.name",
		"app_environment_configs.description",
		"app_environment_configs.value_id",
		"app_environment_configs.environment_id",
		"app_environment_configs",
		"value_id",
		"stage_id",
	},
	"/api.cmdb.AppEnvironmentConfigs/Read": {
		"id",
		"name",
		"description",
		"value_id",
		"environment_id",
	},
	"/api.cmdb.AppEnvironmentConfigs/List": {
		"id",
		"name",
		"description",
		"value_id",
		"environment_id",
	},
	"/api.cmdb.ChartVersions/Read": {
		"id",
		"name",
		"description",
		"repo",
		"version",
	},
	"/api.cmdb.ChartVersions/List": {
		"id",
		"name",
		"description",
		"repo",
		"version",
	},
	"/api.cmdb.ApplicationInstances/Read": {
		"id",
		"name",
		"description",
		"deployment.id",
		"deployment.name",
		"deployment.description",
		"deployment.artifact_id",
		"deployment.kube_cluster_id",
		"deployment.application_instance_id",
		"deployment",
		"value_id",
		"chart_version_id",
		"environment_id",
	},
	"/api.cmdb.ApplicationInstances/List": {
		"id",
		"name",
		"description",
		"deployment.id",
		"deployment.name",
		"deployment.description",
		"deployment.artifact_id",
		"deployment.kube_cluster_id",
		"deployment.application_instance_id",
		"deployment",
		"value_id",
		"chart_version_id",
		"environment_id",
	},
	"/api.cmdb.Vaults/Read": {
		"id",
		"name",
		"description",
		"secrets.id",
		"secrets.name",
		"secrets.description",
		"secrets.path",
		"secrets.type",
		"secrets.key",
		"secrets.vault_id",
		"secrets.value_id",
		"secrets.aws_rds_instance_id",
		"secrets",
	},
	"/api.cmdb.Vaults/List": {
		"id",
		"name",
		"description",
		"secrets.id",
		"secrets.name",
		"secrets.description",
		"secrets.path",
		"secrets.type",
		"secrets.key",
		"secrets.vault_id",
		"secrets.value_id",
		"secrets.aws_rds_instance_id",
		"secrets",
	},
	"/api.cmdb.Secrets/Read": {
		"id",
		"name",
		"description",
		"path",
		"type",
		"key",
		"vault_id",
		"value_id",
		"aws_rds_instance_id",
	},
	"/api.cmdb.Secrets/List": {
		"id",
		"name",
		"description",
		"path",
		"type",
		"key",
		"vault_id",
		"value_id",
		"aws_rds_instance_id",
	},
	"/api.cmdb.AwsServices/Read": {
		"id",
		"name",
		"description",
		"aws_rds_instances.id",
		"aws_rds_instances.name",
		"aws_rds_instances.description",
		"aws_rds_instances.database_host",
		"aws_rds_instances.database_name",
		"aws_rds_instances.database_user",
		"aws_rds_instances.aws_service_id",
		"aws_rds_instances",
		"region_id",
	},
	"/api.cmdb.AwsServices/List": {
		"id",
		"name",
		"description",
		"aws_rds_instances.id",
		"aws_rds_instances.name",
		"aws_rds_instances.description",
		"aws_rds_instances.database_host",
		"aws_rds_instances.database_name",
		"aws_rds_instances.database_user",
		"aws_rds_instances.aws_service_id",
		"aws_rds_instances",
		"region_id",
	},
	"/api.cmdb.AwsRdsInstances/Read": {
		"id",
		"name",
		"description",
		"database_host",
		"database_name",
		"database_user",
		"database_password.id",
		"database_password.name",
		"database_password.description",
		"database_password.path",
		"database_password.type",
		"database_password.key",
		"database_password.vault_id",
		"database_password.value_id",
		"database_password.aws_rds_instance_id",
		"database_password",
		"aws_service_id",
	},
	"/api.cmdb.AwsRdsInstances/List": {
		"id",
		"name",
		"description",
		"database_host",
		"database_name",
		"database_user",
		"database_password.id",
		"database_password.name",
		"database_password.description",
		"database_password.path",
		"database_password.type",
		"database_password.key",
		"database_password.vault_id",
		"database_password.value_id",
		"database_password.aws_rds_instance_id",
		"database_password",
		"aws_service_id",
	},
	"/api.cmdb.Values/Read": {
		"id",
		"name",
		"description",
		"keys.value",
		"keys",
		"aws_rds_instance_id",
	},
	"/api.cmdb.Values/List": {
		"id",
		"name",
		"description",
		"keys.value",
		"keys",
		"aws_rds_instance_id",
	},
	"/api.cmdb.Artifacts/Read": {
		"id",
		"name",
		"description",
		"repo",
		"commit",
		"chart_version_id",
	},
	"/api.cmdb.Artifacts/List": {
		"id",
		"name",
		"description",
		"repo",
		"commit",
		"chart_version_id",
	},
	"/api.cmdb.KubeClusters/Read": {
		"id",
		"name",
		"description",
	},
	"/api.cmdb.KubeClusters/List": {
		"id",
		"name",
		"description",
	},
	"/api.cmdb.Deployments/Read": {
		"id",
		"name",
		"description",
		"artifact.id",
		"artifact.name",
		"artifact.description",
		"artifact.repo",
		"artifact.commit",
		"artifact.chart_version_id",
		"artifact",
		"artifact_id",
		"kube_cluster.id",
		"kube_cluster.name",
		"kube_cluster.description",
		"kube_cluster",
		"kube_cluster_id",
		"application_instance_id",
	},
	"/api.cmdb.Deployments/List": {
		"id",
		"name",
		"description",
		"artifact.id",
		"artifact.name",
		"artifact.description",
		"artifact.repo",
		"artifact.commit",
		"artifact.chart_version_id",
		"artifact",
		"artifact_id",
		"kube_cluster.id",
		"kube_cluster.name",
		"kube_cluster.description",
		"kube_cluster",
		"kube_cluster_id",
		"application_instance_id",
	},
}

func CmdbValidateFiltering(methodName string, f *query.Filtering) error {
	info, ok := CmdbMethodsRequireFilteringValidation[methodName]
	if !ok {
		return nil
	}
	return options.ValidateFiltering(f, info)
}
func CmdbValidateSorting(methodName string, s *query.Sorting) error {
	info, ok := CmdbMethodsRequireSortingValidation[methodName]
	if !ok {
		return nil
	}
	return options.ValidateSorting(s, info)
}
func CmdbValidateFieldSelection(methodName string, s *query.FieldSelection) error {
	info, ok := CmdbMethodsRequireFieldSelectionValidation[methodName]
	if !ok {
		return nil
	}
	return options.ValidateFieldSelection(s, info)
}
